...
namespace tavelapp
{
    ...
    {
        int UserID;
        public BusTicket(string From, string to, string date)
        {
            InitializeComponent();
        ...
        void loadData(string From, string to, string date)
        {
        ...
        private Panel CreatePanel(string no, string Price ,string status,string Bid)
        {
            ...
            button.Click += (sender, id) =>
            {
                ...
                {
                    getid();
                    SqlCommand cmd = new SqlCommand($"INSERT INTO BusBooking (UserID, RoomID) VALUES ('{UserID}', '{Bid}')", Program.con);
                    Program.con.Open();
                    cmd.ExecuteNonQuery();
                    Program.con.Close();

                    SqlCommand cmd2 = new SqlCommand($"UPDATE Bus SET Status = 'Reserved' WHERE BusID = '{Bid}'", Program.con);
                    Program.con.Open();
