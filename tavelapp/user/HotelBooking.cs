...
namespace tavelapp
{
    public partial class HotelBooking : Form
    {
        ...
        void location()
        {
            ...
            ddllocation.DisplayMember = dt.Columns[3].ToString();
        }
        void loadData()
        {
            SqlCommand cmd = new SqlCommand("select * from Hotel", Program.con);
            ...
            {
                flowLayoutPanel1.Controls.Add(CreatePanel(dt.Rows[i][2].ToString(), dt.Rows[i][3].ToString(), dt.Rows[i][5].ToString(), dt.Rows[i][0].ToString()));

            }
        }
        private Panel CreatePanel(string name, string location, string Img,string hotel)
        {
            ...
            button.Click += (sender, id) =>
            {
                ...
                RoomBooking roomBooking = new RoomBooking(int.Parse(hotel));
                this.Hide();
        ...
        {
            SqlCommand cmd = new SqlCommand($"select * from Hotel where Location = '{ddllocation.SelectedValue.ToString()}' ", Program.con);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
            for (int i = 0; i < dt.Rows.Count; i++)
